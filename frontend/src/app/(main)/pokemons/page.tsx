...
const pokemonTypes: Array<PokemonType> = [
  ...
  'steel', 'grass', 'fire', 'water', 'electric', 'psychic', 'ice', 'dragon',
  'dark', 'fairy',
];
...
export default function Pokemons() {
  ...
  const toggleDropdown = () => {
    showFilters === MODAL_FILTRO.NONE ? setShowFilters(MODAL_FILTRO.LIST) : setShowFilters(MODAL_FILTRO.NONE);
  };
                ...
                <div className="py-1">
                  {generations.map((item) => (
                    <button
                      key={item.id}
                      className="block w-full text-right px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                      onClick={() => setFilter(`gen-${item.id}`)} // Atualiza o filtro com a geração
                    >
                      {item.nome}
                    </button>
                  ))}
                ...
                <div className="py-1">
                  {pokemonTypes.map((tipo, index) => (
                    <button
                      ...
                      onClick={() => setFilter(`tipo=${tipo}`)} // Atualiza o filtro com o tipo
                    >
